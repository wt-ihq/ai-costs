...
- **OpenRouter** — `POST /analytics/query` (dimensions `["user","model"]` + day granularity; the `date__day` bucket doesn't count against the 2-dimension cap) is the fact source, queried **once per workspace** (`workspace` is allowed as a *filter*, giving workspace × user × model grain): member rows sum org-wide to email entities (→ employee); keyless rows (workspace-owned API keys) become entity_key = workspace name with the department from `openrouter_workspaces` (auto-registered, department **prefilled with the workspace name** — the org names workspaces after departments; admins correct exceptions on Data → Tools). Falls back to one org-wide query if `/workspaces` fails. Per-day USD + tokens + requests; the first source to populate the `tokens`/`requests` columns. Metric values may arrive as **strings** — parse defensively; a `metadata.truncated` response **throws** rather than persisting a partial window. `GET /activity` (last 30 completed UTC days, exact charged USD) is the drift check — any day it reports more than analytics gets the difference as an `unkeyed` fact. Both need `OPENROUTER_MANAGEMENT_KEY` (a **management key**, Settings → Management Keys — not an inference key, which 403s). BYOK spend (`byok_usage_inference`) is deliberately excluded (would double-count the provider's own facts). Dedicated `/openrouter` page; lifetime sanity check: sum of facts ≤ `GET /credits` `total_usage`.
- **ChatGPT Business / Claude Team (manual imports)** — no API; admins paste from the vendor dashboards on the Imports page. Each import is a **monthly snapshot-replace** stamped to the "Month" field's calendar month. **Export a Custom range covering exactly one calendar month** — ChatGPT's 1M/6M/12M presets are *rolling* windows and double-count across months, and the paste carries no dates so nothing downstream can detect it (gotcha #3 applies). Re-importing the same month is always safe (replace). The Imports page's coverage table shows which months each source has.
...
- **Cache layer** (`src/lib/queries/cached.ts` — heavy reads cached via `unstable_cache` under `FACTS_TAG`, hourly safety revalidate). Two separate invariants:
  - *Write paths:* any new server action or route writing `spend_facts`, `employees`, `seat_month_entries`, `recurring_costs`, `vercel_projects`, or `seat_assignments` must bust the tag — `updateTag(FACTS_TAG)` in Server Actions (read-your-own-writes), `revalidateTag(FACTS_TAG, "max")` in route handlers like the cron. This Next 16 build **requires** the second profile arg on `revalidateTag`; the one-arg form is `updateTag`, Server-Action-only. Otherwise admins see stale data for up to an hour. New heavy reads belong in `cached.ts`, not fetched raw in pages.
  - *Shape changes:* changing the shape of anything a `*Cached` reader returns — **adding a field counts** — MUST bump `CACHE_VERSION` in the same commit. Cache entries survive deploys, so the new build reads old-shaped entries and the page throws on every cache **HIT**. Tests, lint and `CI=true npm run build` all pass regardless (the stale entry exists only in prod), so this is a checklist item, not something testing catches. Versioning exists *because* this crashed once already (c82bb34, `unpackScope` on packed scopes, introduced at v2) and it crashed again when the rule was forgotten (8f2eac7 → v5). The bumps that went right — b105d96 → v3, ce1837d → v4 — were made in the same commit as the shape change. Do that.
- Commit messages end with: `Co-Authored-By: Claude <model name> (1M context) <noreply@anthropic.com>` — use the model actually doing the work (e.g. `Claude Opus 5 (1M context)`), not whichever version this line was last updated for.
