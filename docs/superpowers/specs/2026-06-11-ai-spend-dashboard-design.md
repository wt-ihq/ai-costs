...
- **Currency: Claude Team bills in GBP**, conflicting with the §2/§10 "USD-only, no FX" scope. Decision (2026-06-15): convert £→$ at ingest via a **fixed admin-configured rate** (`fx_rates.GBP`, not live FX), keeping `spend_facts` single-currency USD; native £ retained in `raw_payloads`. `buildClaudeSpendFacts` applies the rate and keys each user's MTD to the 1st of the month (upsert-replace). Rate seeded at 1.27 — set the real business rate.
